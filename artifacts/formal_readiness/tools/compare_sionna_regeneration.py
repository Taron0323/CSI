...
def _required_arrays_exact(
    reference: dict[str, np.ndarray],
    regenerated: dict[str, np.ndarray],
    required: set[str],
) -> bool:
    for name in required:
        if name not in reference or name not in regenerated:
            return False
        ...
        if (
            ...
            and (not np.isfinite(left).all() or not np.isfinite(right).all())
        ):
            return False
...
def main() -> None:
    ...
    payload = {
        ...
        "status": (
            "PASS"
            if _required_arrays_exact(reference, regenerated, regenerated_required)
            else "FAIL"
        ),
