...
G8. Supply a genuinely different engine through the direct precomputed-archive
schema documented in `../A100_RUNBOOK.md`. That path authenticates the raw CSI,
independent-engine scene manifest, and actual engine configuration, then uses
the same first-party outer statistics and claim reauthentication. It does not
accept self-reported aggregate effects or a renamed Sionna rerender.
