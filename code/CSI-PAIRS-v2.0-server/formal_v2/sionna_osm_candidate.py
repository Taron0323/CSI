...
CONFIG_SCHEMA = "csi-pairs-sionna-osm-candidate-config-v2"
ASSET_SCHEMA = "csi-pairs-sionna-osm-asset-manifest-v2"
...
def load_config(path: str | Path) -> dict:
    ...
    if (
        ...
        or 2 * int(map_config["receiver_primitive_quota"]) > int(config["positions_per_bank"])
    ):
...
def _select_city_banks(
    ...
    chosen = _select_nonoverlapping_bank_candidates(
        candidates,
        required,
...
def render_bank(bank: dict, asset_root: Path, config: dict, scene_index: int) -> dict[str, np.ndarray]:
    ...
    if np.any(np.all(path_ids < 0, axis=-1)) or np.any(np.all(noop_ids < 0, axis=-1)):
        raise RuntimeError(
            f"pre-render visibility contract failed for bank {bank['scene_id']}"
        )
...
def _receiver_positions(bank: dict, config: dict, scene_index: int) -> np.ndarray:
    ...
    radio = config["radio"]
    buildings = [
        (Polygon(row["local_exterior_xy_m"]), float(row["height_m"]))
        for row in bank["buildings"]
    ]
    polygons = [row[0] for row in buildings]
    ...
    for y in values + offset_y:
        for x in values + offset_x:
            if math.hypot(float(x), float(y)) < minimum_distance:
                continue
            point = Point(float(x), float(y))
            if not all(float(polygon.distance(point)) >= clearance for polygon in polygons):
                continue
            line = LineString(((0.0, 0.0), (float(x), float(y))))
            if _has_direct_geometric_visibility(
                line,
                buildings,
    ...
    for primitive in primitives:
        ...
        take = ranked[: min(quota, count - len(selected))]
        selected.extend(take)
        remaining.difference_update(take)
    active = np.zeros(len(candidates), dtype=np.bool_)
    active[list(remaining)] = True
    minimum_squared_distance = np.full(len(candidates), np.inf, dtype=np.float64)
    ...
    while len(selected) < count:
        ...
        if not np.isfinite(minimum_squared_distance[index]):
            raise RuntimeError("receiver maximin sampler exhausted visible candidates")
    ...
    output = candidate_array[np.asarray(selected, dtype=np.int64)]
...
def build_engine_config(asset_root: Path, manifest: dict, config: dict) -> dict:
    return {
        "profile": "formal-candidate-sionna-rt-osm-urban-banks-v2",
        "simulation_not_measurement": True,
        "fixture": False,
        "scientific_use": "CANDIDATE",
        ...
        "geometry": {
            ...
            "bank_selection": "score-ranked disjoint 256-meter scene extents within each city",
            ...
            "receiver_sampling": {
                ...
                "selection": "3D direct-visibility filter, primitive proximity quotas, deterministic maximin fill",
                ...
                "primitive_quota": int(config["map"]["receiver_primitive_quota"]),
            },
        },
        "generator": {
