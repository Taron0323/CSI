...
def _sionna_bootstrap_environment(
    ...
    *,
    ...
    if backend not in {"cuda", "llvm"}:
        raise ValueError(f"unsupported Sionna backend: {backend!r}")
    root = Path(project_root).resolve()
    environment = dict(os.environ if current is None else current)
    if backend == "llvm":
        python = Path(sys.executable)
        llvm_value = libllvm or environment.get("DRJIT_LIBLLVM_PATH")
        if not llvm_value:
            raise RuntimeError(
                "Sionna LLVM runtime requires DRJIT_LIBLLVM_PATH to identify libLLVM"
            )
        llvm_path = Path(llvm_value).expanduser().resolve()
        required = {
        ...
        llvm_path = Path(SIONNA_LIBLLVM if libllvm is None else libllvm)
        required = {
            "Sionna runtime Python": python,
    ...
    if backend == "cuda":
        preload = [
            value for value in environment.get("LD_PRELOAD", "").split(":") if value
        ]
...
def ensure_sionna_runtime(reexec_arguments: list[str], *, backend: str = "cuda") -> None:
    project_root = Path(__file__).resolve().parents[1]
    python, environment = _sionna_bootstrap_environment(project_root, backend=backend)
    ...
    if backend == "cuda":
        ...
        backend_ready = str(SIONNA_CUDA_DRIVER) in preload_entries
    else:
        active_llvm = os.environ.get("DRJIT_LIBLLVM_PATH")
        backend_ready = bool(active_llvm) and Path(str(active_llvm)).resolve() == Path(
            environment["DRJIT_LIBLLVM_PATH"]
        ).resolve()
    if correct_python and backend_ready:
        if backend == "llvm" and platform.python_version() != SIONNA_PYTHON_VERSION:
            raise RuntimeError(
                "Sionna LLVM runtime requires Python "
                f"{SIONNA_PYTHON_VERSION}, got {platform.python_version()}"
            )
        return
    if os.environ.get("CSI_PAIRS_SIONNA_BOOTSTRAPPED") == "1":
        requirement = (
            "fixed Python and CUDA driver"
            if backend == "cuda"
            else "fixed Python and Dr.Jit LLVM library"
        )
        raise RuntimeError(
            f"Sionna runtime bootstrap did not activate the {requirement}"
        )
...
def _sionna_runtime_record(*, backend: str = "cuda") -> dict:
    if backend not in {"cuda", "llvm"}:
        raise ValueError(f"unsupported Sionna backend: {backend!r}")
    import mitsuba
    ...
    if not llvm_value:
        raise RuntimeError("Sionna LLVM runtime requires DRJIT_LIBLLVM_PATH")
    llvm_path = Path(llvm_value).expanduser().resolve()
    if not llvm_path.is_file():
        raise RuntimeError(f"Dr.Jit LLVM library is missing: {llvm_path}")
    return {
        **common,
        "backend": "llvm",
