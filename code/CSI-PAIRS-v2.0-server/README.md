...
The latter can be supplied without changing first-party code through the
authenticated precomputed-RT archive contract in `formal_v2/A100_RUNBOOK.md`;
preflight binds raw CSI, every independent scene asset, and the actual engine
configuration before the outer G8 statistics are recomputed.
