...
- 12 个 catalog `COPIED` 实体全部存在，数据根均为真实目录而非顶层软链接。
...
- `/System/Volumes/Data`：926 GiB 总量、540 GiB 已用、349 GiB 可用、61%。
...
这次 `PASS` 证明本地副本、哈希、规模和登记的失败状态一致；它不证明 RT qualification、paired intervention 资格、无泄漏、四臂结论或 external validity。科学缺口见套件根 `MISSING_ITEMS.md`。
