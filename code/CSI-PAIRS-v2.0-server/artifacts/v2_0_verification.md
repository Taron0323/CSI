...
|---|---|---|
| Static compilation | `python3 -m py_compile formal_v2/*.py formal_v2/tests/test_formal_v2.py` | PASS |
| Static lint | `ruff check formal_v2` | PASS |
| Pure schema/semantic tests | `python3 -m unittest discover -s formal_v2/tests -v` | 52/52 PASS |
...
- scene-ID, shuffled-pair, retention, resource-control, external-model, RT-calibration, literature-resource, or external-validity adapters;
...
Formal execution still requires a qualified non-fixture dataset, an independent renderer adapter and version/license binding, RT calibration evidence, actual external model adapters, resource-control implementations, and a second engine or controlled intervention for G8. The inherited No-X/null failures and the four named warnings remain binding until untouched non-fixture evidence replaces them through the registered gates.
