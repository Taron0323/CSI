...
`FORMAL_INPUT_READY=BLOCKED`, `LAUNCH_READY=BLOCKED`, and
`SCIENTIFIC_EVIDENCE=NOT_ASSESSED` are mandatory until every P0 row below closes.

| ID | Severity/type | Missing decision or input | Acceptance check | Consequence |
|---|---|---|---|---|
| `SC-GAUGE-001` | P0 `AUTHOR_DECISION_REQUIRED` | Freeze shared RT complex reference, a fully specified source-derived transform/inverse, or a named phase-invariant representation. | Update frozen protocol, config/data contract, tests and paper as one change. | G1, paper protocol and formal launch remain blocked. |
| `SC-ROUTE-002` | P0 `AUTHOR_DECISION_REQUIRED` | Confirm whether native full-channel mask-cover remains on `r^A` while the unified patch probe uses `r^{R,q}`, or replace the native estimand with an explicitly defined assembly rule. | One unambiguous formula and matching code/config/test/paper contract. | Response protocol and formal launch remain blocked. |
...
- `FORMAL_GO=NO-GO`: do not start formal Stage-0, Wi-GATr training, four-arm training, two-city
  evaluation or G8 until the two author decisions and every required external preflight row close.
