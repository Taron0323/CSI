...
|---|---|---|---|---|
| `INPUT-DATA-001` | P0 `DEEP_VERIFICATION_AND_RUNTIME_REVIEW_REQUIRED` | The repository has immutable commitments and derived inventories, but omits the candidate/manifests. The libLLVM hash now matches the registry, while the bound generation source predates approval enforcement and cannot prove pre-approval. | Deep mode authenticates all external roots and exact arrays; data are regenerated with a bound approved-runtime record. | The reported candidate cannot enter G1/G2. |
