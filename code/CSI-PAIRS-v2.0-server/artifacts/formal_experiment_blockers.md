...
|---|---|---|---|---|
| `INPUT-RT-001` | P0 `EXTERNAL_DATA_REQUIRED` | Independent RT calibration fit/validation/reference manifests, shared reference evidence and raw rows. Candidate-data regeneration is complete, but it does not substitute for the separate calibration partitions. | `run-rt-calibration` plus G1 four-statistic/noise-floor checks pass. | C11 and qualification remain blocked. |
...
|---|---|---|
| `INPUT-DATA-001` | local 34-bank candidate gate | Non-fixture V2.1 candidate `e5ec3d32...7847` contains 34 banks, four paired worlds and 256 positions per bank. `inspect-data` passes; independent regeneration passes 34/34 per-scene rows and all nine role groups at `rtol=0`, `atol=0`, with matching engine configuration and zero nonblocking failures. It remains `scientific_use=CANDIDATE_NOT_CLAIM`, not paper evidence. |
